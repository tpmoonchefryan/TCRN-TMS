...
const LEGACY_TEXT_OVERRIDES: Record<string, LocalizedText> = {
  ...
  'Test email address': buildExactText(
    'Test email address',
    '测试邮箱地址',
    '測試信箱地址',
    'テストメールアドレス',
  ...
  'Test connection': buildExactText(
    'Test connection',
    '测试连接',
    '測試連線',
    '接続をテスト',
  ...
  'Send test email': buildExactText(
    'Send test email',
    '发送测试邮件',
    '傳送測試郵件',
    'テストメールを送信',
  ...
  'Email Templates': buildExactText(
    'Email Templates',
    '邮件模板',
    '郵件模板',
    'メールテンプレート',
  ...
  'Template Detail': buildExactText(
    'Template Detail',
    '模板详情',
    '模板詳情',
    'テンプレート詳細',
  ...
  'Select a scope first': buildExactText(
    'Select a scope first',
    '请先选择一个范围',
    '請先選擇一個範圍',
    '先にスコープを選択してください',
    '먼저 범위를 선택하세요',
    'Selectionnez d abord un perimetre'
  ),
};
...
export function pickIntegrationText(locale: SupportedUiLocale, value: LocalizedText) {
...
export function useIntegrationManagementCopy() {
  ...
  return {
    ...
    tabLabel: (tab: IntegrationTab) =>
      resolveLocalizedText(effectiveSelectedLocale, TAB_LABELS[tab]),
    adapterTypeLabel: (type: AdapterType) =>
      resolveLocalizedText(effectiveSelectedLocale, ADAPTER_TYPE_LABELS[type]),
