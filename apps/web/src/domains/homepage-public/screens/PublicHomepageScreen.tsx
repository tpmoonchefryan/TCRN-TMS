...
export async function generateMetadata(
  ...
  const tHomepage = await getTranslations('publicHomepage');
  ...
  if (!data) {
    return {
      title: tHomepage('notFoundTitle'),
    };
  }

  const talentName = data.talent?.displayName || tHomepage('defaultTalentName');
  const title = data?.seo?.title || tHomepage('defaultSeoTitle', { talentName });
  const description = data?.seo?.description || tHomepage('defaultSeoDescription', { talentName });
...
export async function PublicHomepageScreen(props: PageProps) {
  const [tLegal, tCommon] = await Promise.all([
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <div
        className="pointer-events-none fixed bottom-4 left-0 right-0 z-50 select-none text-center text-[10px] opacity-40 mix-blend-difference"
        style={{ color: data.theme?.colors?.text || '#000' }}
      >
        {tLegal('poweredBy')} {tCommon('appName')}
      </div>
    </div>
  );
}
