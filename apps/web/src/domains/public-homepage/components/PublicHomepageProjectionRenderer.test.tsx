...
describe('PublicHomepageProjectionRenderer', () => {
  ...
  it('renders projection-safe hero, links, and structured rich text', () => {
    renderWithLocale(<PublicHomepageProjectionRenderer projection={baseProjection} />);

    expect(
      screen.getByRole('heading', { level: 1, name: 'Hoshimachi Suisei' })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByRole('link', { name: 'YouTube' })[0].querySelector('svg')
    ).toHaveAttribute('aria-hidden', 'true');
    expect(screen.getByRole('img', { name: 'Hoshimachi Suisei avatar' })).toHaveAttribute(
      'loading',
      'eager'
    );
    expect(screen.getByRole('img', { name: 'Hoshimachi Suisei avatar' })).toHaveAttribute(
  ...
  it('localizes projection fallback labels and sentinel defaults for zh locale', () => {
    ...
    expect(
      screen.getByText('在这里查看倒计时动态、揭晓时刻和面向粉丝的上线入口。')
    ).toBeInTheDocument();
  ...
  it('forces compact hero layout when preview surfaces request mobile mode', () => {
    renderWithLocale(
      <PublicHomepageProjectionRenderer projection={baseProjection} responsiveMode="mobile" />
    );
  ...
  it('uses fan-facing grouped action descriptions instead of design-rationale copy', () => {
    ...
    expect(
      screen.getByText('Start with the stream, archive, or next moment waiting for you here.')
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        'More ways to cheer, follow updates, and stay close to the next stream live here.'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText('Membership, shop, and other official support links are all together here.')
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/keep the one most important next step easy to scan/i)
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/stay grouped so the page keeps a cleaner first scan/i)
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/official support paths stay grouped together/i)
    ).not.toBeInTheDocument();
  });
});
