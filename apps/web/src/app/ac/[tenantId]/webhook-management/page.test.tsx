...
import AcWebhookManagementPage from './page';

const mockRequest = vi.fn();

vi.mock('next/navigation', () => ({
  usePathname: () => '/ac/tenant-ac/webhook-management',
  useRouter: () => ({
    replace: vi.fn(),
  }),
...
describe('AC webhook-management direct route', () => {
  beforeEach(() => {
    mockRequest.mockReset();
    mockRequest.mockRejectedValue(new Error('Operational integration request should not run'));
  });

  it('renders not_available_in_ac before operational webhook effects run', async () => {
    render(
      await AcWebhookManagementPage({
    ...
    expect(screen.queryByRole('button', { name: /refresh|test|replay/i })).not.toBeInTheDocument();
