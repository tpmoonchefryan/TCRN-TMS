...
describe('RequirePermissions', () => {
  it('accepts catalog-backed resources and alias actions', () => {
    ...
    expect(Reflect.getMetadata(PERMISSIONS_KEY, TestController.prototype.handler)).toEqual([
      { resource: 'customer.export', action: 'create' },
    ]);
  });

  it('rejects unknown RBAC resource codes at declaration time', () => {
    expect(() => RequirePermissions({ resource: 'config.unknown' as any, action: 'read' })).toThrow(
      'Unknown RBAC resource code: config.unknown'
    );
  });
