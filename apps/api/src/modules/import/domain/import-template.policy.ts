...
export interface ImportErrorCsvRow {
  rowNumber: number;
  errorCode: string;
  errorMessage: string;
  originalData: string;
...
export const validateCsvTemplate = (
  ...
  const expectedHeaders = [...getExpectedHeaders(templateKind)];
  const errors: string[] = [];
...
export const generateCompanyImportTemplate = (): string => {
  const exampleRow = [
    'EXT002',
    '合作方A',
    'ABC科技有限公司',
...
export const generateImportErrorsCsv = (errors: ImportErrorCsvRow[]): string => {
