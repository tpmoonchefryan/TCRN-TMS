...
import { CreateMfrJobDto, MfrSearchRequestDto, ReportJobListQueryDto } from '../dto/report.dto';
...
const MFR_JOB_LIST_ITEM_SCHEMA = {
  ...
  properties: {
    ...
    completedAt: {
      ...
      nullable: true,
      format: 'date-time',
      example: '2026-04-13T10:02:00.000Z',
    },
    expiresAt: {
      type: 'string',
      nullable: true,
      format: 'date-time',
      example: '2026-04-13T10:07:00.000Z',
    },
  },
  required: [
    'id',
...
const MFR_JOB_DETAIL_SCHEMA = {
  ...
  properties: {
    ...
    startedAt: {
      ...
      nullable: true,
      format: 'date-time',
      example: '2026-04-13T10:00:10.000Z',
    },
    completedAt: {
      type: 'string',
      nullable: true,
...
export class ReportController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns MFR search preview data',
    schema: MFR_SEARCH_RESULT_SCHEMA,
  })
  @ApiResponse({
    status: 400,
    description: 'MFR search payload is invalid',
    schema: REPORT_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to search MFR data',
    schema: REPORT_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to search MFR data',
    schema: REPORT_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
    description: 'Talent or report source data was not found',
    schema: createErrorEnvelopeSchema(ErrorCodes.RES_NOT_FOUND, 'Talent not found'),
  })
  async searchMfr(
    ...
    return this.mfrReportService.search(dto.talentId, dto.filters, dto.previewLimit, context);
  }
  ...
  @ApiResponse({
    status: 201,
    description: 'MFR export job created or handed off to TCRN PII Platform',
    schema: MFR_CREATE_RESPONSE_SCHEMA,
  })
  @ApiResponse({
    status: 400,
    description: 'MFR export request is invalid',
    schema: REPORT_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to create MFR export jobs',
    schema: REPORT_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to create MFR export jobs',
    schema: REPORT_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
    ...
    schema: createErrorEnvelopeSchema(
      ErrorCodes.RES_NOT_FOUND,
      'Talent not found or has no profile store'
    ),
  })
  async createMfrJob(
    ...
    const context = this.buildContext(user, req);
    return this.mfrReportService.createJob(dto.talentId, dto.filters, dto.format, context);
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns paginated MFR jobs',
    schema: MFR_JOB_LIST_SCHEMA,
  })
  @ApiResponse({
    status: 400,
    description: 'MFR job query is invalid',
    schema: REPORT_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to list MFR jobs',
    schema: REPORT_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to list MFR jobs',
    schema: REPORT_FORBIDDEN_SCHEMA,
  })
  async listMfrJobs(
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns MFR job detail',
    schema: MFR_JOB_DETAIL_SCHEMA,
  })
  @ApiResponse({
    status: 400,
    description: 'MFR job lookup query is invalid',
    schema: REPORT_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to read MFR jobs',
    schema: REPORT_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to read MFR jobs',
    schema: REPORT_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
    description: 'MFR job was not found',
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns MFR report download URL',
    schema: MFR_JOB_DOWNLOAD_SCHEMA,
  })
  @ApiResponse({
    status: 400,
    description: 'Report download is not currently available',
    schema: createErrorEnvelopeSchema(
      ErrorCodes.VALIDATION_FAILED,
  ...
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to download MFR reports',
    schema: REPORT_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to download MFR reports',
    schema: REPORT_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 400,
    description: 'MFR job cannot be cancelled in its current state',
    schema: createErrorEnvelopeSchema(
      ErrorCodes.VALIDATION_FAILED,
  ...
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to cancel MFR jobs',
    schema: REPORT_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to cancel MFR jobs',
    schema: REPORT_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
