...
export class ExportJobReadRepository {
  ...
  async findById(tenantSchema: string, jobId: string): Promise<RawExportJobRecord | null> {
  ...
  async countMany(tenantSchema: string, filters: ExportJobListFilters): Promise<number> {
    const { whereClause, params } = this.buildListQuery(filters);
