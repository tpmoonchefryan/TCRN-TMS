...
export class IntegrationLogController {
  constructor(private readonly integrationLogService: IntegrationLogQueryService) {}
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns paginated integration logs',
    schema: INTEGRATION_LOG_LIST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to read integration logs',
    schema: LOG_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to read integration logs',
    schema: LOG_FORBIDDEN_SCHEMA,
  })
  async list(@Query() query: IntegrationLogQueryDto, @CurrentUser() user: AuthenticatedUser) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns logs for trace',
    schema: INTEGRATION_LOG_TRACE_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to read integration logs by trace',
    schema: LOG_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to read integration logs by trace',
    schema: LOG_FORBIDDEN_SCHEMA,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns failed requests',
    schema: INTEGRATION_LOG_LIST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to read failed integration requests',
    schema: LOG_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to read failed integration requests',
    schema: LOG_FORBIDDEN_SCHEMA,
  })
  async getFailed(@Query() query: PaginationDto, @CurrentUser() user: AuthenticatedUser) {
