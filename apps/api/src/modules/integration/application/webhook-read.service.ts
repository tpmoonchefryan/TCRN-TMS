...
import { mapWebhookDetail, mapWebhookListItem } from '../domain/webhook.policy';
...
export class WebhookReadApplicationService {
  constructor(private readonly webhookReadRepository: WebhookReadRepository) {}

  async findMany(context?: RequestContext) {
    const webhooks = await this.webhookReadRepository.findMany(getWebhookTenantSchema(context));

    return webhooks.map((webhook) => mapWebhookListItem(webhook));
  }

  async findById(id: string, context?: RequestContext) {
    const webhook = await this.webhookReadRepository.findById(id, getWebhookTenantSchema(context));
