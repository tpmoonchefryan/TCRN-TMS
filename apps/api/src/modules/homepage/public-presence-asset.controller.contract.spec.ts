...
describe('PublicPresenceAssetController route contract', () => {
  ...
  it('keeps asset-scoped CRUD under /public-presence/assets', () => {
    ...
    expect(getControllerRoutes(PublicPresenceAssetController)).toEqual(
      expect.arrayContaining([
        ...
        { methodName: 'listAssetRevisions', requestMethod: RequestMethod.GET, path: ':assetId/revisions' },
        { methodName: 'createAsset', requestMethod: RequestMethod.POST, path: '/' },
        { methodName: 'saveAssetDraft', requestMethod: RequestMethod.PUT, path: ':assetId/current' },
        { methodName: 'validateAsset', requestMethod: RequestMethod.POST, path: ':assetId/current/validate' },
        { methodName: 'duplicateAsset', requestMethod: RequestMethod.POST, path: ':assetId/duplicate' },
      ]),
    );
  });
  ...
  it('protects validation writes separately from document writes', () => {
    expect(Reflect.getMetadata(
      PERMISSIONS_KEY,
      controllerClass.prototype.listAssets,
    )).toEqual([{ resource: 'public_presence.document', action: 'read' }]);

    expect(Reflect.getMetadata(
      PERMISSIONS_KEY,
      controllerClass.prototype.createAsset,
    )).toEqual([{ resource: 'public_presence.document', action: 'write' }]);

    expect(Reflect.getMetadata(
      PERMISSIONS_KEY,
      controllerClass.prototype.validateAsset,
    )).toEqual([{ resource: 'public_presence.validation', action: 'write' }]);
  });
});
