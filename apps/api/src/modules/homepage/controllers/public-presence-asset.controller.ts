...
export class PublicPresenceAssetController {
  ...
  @ApiOperation({ summary: 'List Public Presence template/component assets visible in the current scope' })
  ...
  async listAssets(
    ...
    return this.publicPresenceAssetService.listAssets(
      user.tenantSchema,
      query,
      user.id,
    );
  ...
  @ApiOperation({ summary: 'Create a scoped Public Presence asset with a non-empty starter source bundle' })
  async createAsset(
  ...
  @Put(':assetId/current')
  @RequirePermissions({ resource: 'public_presence.document', action: 'write' })
  @ApiOperation({ summary: 'Persist a new current draft revision for one scoped Public Presence asset' })
  ...
  @ApiOperation({ summary: 'Persist a validated current revision for one scoped Public Presence asset' })
  async validateAsset(
    @Param('assetId', ParseUUIDPipe) assetId: string,
    @Body(zodPipe(UpdatePublicPresenceAssetRevisionSchema))
    dto: UpdatePublicPresenceAssetRevisionDto,
  ...
  @RequirePermissions({ resource: 'public_presence.document', action: 'write' })
  @ApiOperation({ summary: 'Duplicate the current asset revision into the current Entity Management scope' })
