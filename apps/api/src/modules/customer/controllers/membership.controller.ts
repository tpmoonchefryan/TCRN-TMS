...
export class MembershipController {
  constructor(private readonly membershipRecordService: MembershipRecordService) {}
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns membership records',
    schema: MEMBERSHIP_LIST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to read membership records',
    schema: CUSTOMER_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to read membership records',
  ...
  @ApiResponse({
    status: 400,
    description: 'Membership payload is invalid',
    schema: CUSTOMER_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to add membership records',
    schema: CUSTOMER_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to add membership records',
    schema: CUSTOMER_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 400,
    description: 'Membership update is invalid',
    schema: CUSTOMER_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to update membership records',
    schema: CUSTOMER_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to update membership records',
    schema: CUSTOMER_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
    description: 'Membership record or dependent resource was not found',
    schema: CUSTOMER_NOT_FOUND_SCHEMA,
  })
  async update(
    @Param('talentId', ParseUUIDPipe) talentId: string,
    ...
    const context = this.buildContext(user, req);
    return this.membershipRecordService.update(customerId, recordId, talentId, dto, context);
  }
