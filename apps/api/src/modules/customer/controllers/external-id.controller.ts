...
export class ExternalIdController {
  constructor(private readonly externalIdService: CustomerExternalIdService) {}
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns external IDs',
    schema: CUSTOMER_EXTERNAL_ID_LIST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to read customer external IDs',
    schema: CUSTOMER_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to read customer external IDs',
  ...
  @ApiResponse({
    status: 201,
    description: 'External ID added',
    schema: CUSTOMER_EXTERNAL_ID_ITEM_SCHEMA,
  })
  @ApiResponse({
    status: 400,
    description: 'External-ID payload is invalid',
    schema: CUSTOMER_BAD_REQUEST_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to add external IDs',
    schema: CUSTOMER_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to add external IDs',
    schema: CUSTOMER_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
    description: 'Customer or consumer was not found',
    schema: CUSTOMER_NOT_FOUND_SCHEMA,
  })
  @ApiResponse({
    status: 409,
    description: 'External ID already exists for this customer',
  ...
  @ApiParam({
    name: 'externalIdId',
    description: 'External-ID identifier',
    schema: { type: 'string', format: 'uuid' },
  })
  @ApiResponse({
    status: 200,
    description: 'External ID deleted',
    schema: CUSTOMER_EXTERNAL_ID_DELETE_SCHEMA,
  })
  @ApiResponse({
    status: 401,
    description: 'Authentication is required to delete external IDs',
    schema: CUSTOMER_UNAUTHORIZED_SCHEMA,
  })
  @ApiResponse({
    status: 403,
    description: 'Insufficient permissions to delete external IDs',
    schema: CUSTOMER_FORBIDDEN_SCHEMA,
  })
  @ApiResponse({
    status: 404,
    description: 'Customer or external ID was not found',
    schema: CUSTOMER_NOT_FOUND_SCHEMA,
  })
  async delete(
    @Param('talentId', ParseUUIDPipe) talentId: string,
