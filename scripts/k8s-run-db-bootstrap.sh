...
# Run the first-install database bootstrap job for K3s.

set -euo pipefail

NAMESPACE="${NAMESPACE:-tcrn}"
JOB_NAME="tcrn-db-bootstrap"
...
MANIFEST_PATH="infra/k8s/jobs/db-bootstrap.yaml"

if [[ -z "${IMAGE_TAG}" ]]; then
  echo "Usage: IMAGE_TAG=<tag> $0"
  echo "   or: $0 <tag>"
  exit 1
fi
...
sed \
  "s|ghcr.io/tpmoonchefryan/tcrn-tms/api:replace-me|${IMAGE_REPO}:${IMAGE_TAG}|g; s|namespace: tcrn|namespace: ${NAMESPACE}|g" \
