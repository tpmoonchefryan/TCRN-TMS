...
function classify(t) {
  if (/Clean|Blocker-aware|Owner-accepted|Excluded|proof slice|not yet|G19|OKL-/i.test(t)) return "proof-status";
  if (/^[A-Z][a-z]+ [a-z]+ (page|screen|console|studio)|page|screen|route|navigation|menu|tab\b/i.test(t)) return "ui-surface";
  if (/API|endpoint|webhook|request|response|POST|GET|OpenAPI|adapter/i.test(t)) return "api-contract";
...
L.push("# 文档声称清册");
L.push("");
L.push("> 治理记录:`TCRN-TMS-STORY-020`(Epic `TCRN-TMS-EPIC-006`)· 分解裁定 minutes `TCRN-TMS-MIN-001` · 门 `TCRN-TMS-GATE-003`");
L.push(">");
L.push("> **本文件由 `scripts/facts-generate-claims-inventory.mjs` 生成,请勿手改。**");
L.push("");
L.push("## 范围");
L.push("");
L.push("`docs/user-guide/` 与 `docs/wiki-draft/` 共 **" + files.length + " 个文件、" + totalLines + " 行**,抽出 **" + claims.length + " 条**候选声称(markdown 列表项)。");
L.push("");
L.push("每条声称获得一个稳定 id `TMS-C-NNN`,供三批审计(`TCRN-TMS-STORY-021`~`023`)逐条判定引用。");
L.push("");
L.push("> **id 稳定性的边界**:编号按文件名排序、行号顺序生成。**审计过程中修改文档会使其后所有 id 位移。** 故审计须以「一批一次重生成、批内引用当次 id」的方式进行,不可跨批沿用旧号。");
L.push("");
...
L.push("");
L.push("**分类是启发式的**(关键词匹配),不是判定。它只决定每条声称该拿哪把尺子量;审计时须逐条复核归类本身是否正确。");
L.push("");
L.push("### 两个需要立刻说清的类别");
L.push("");
L.push("**`proof-status`(" + (tally["proof-status"] || 0) + " 条)不是产品事实。** 这些行描述的是旧证明流程的状态(`Clean` / `Blocker-aware` / `Owner-accepted` / `Excluded` / `G19` / `OKL-*`),不是系统的行为。其支撑证据随旧 vault 于 2026-07-20 归档移出本仓,故 `OKL-*` 限制 ID 现为孤儿。处置见 `TCRN-TMS-STORY-024`。");
L.push("");
L.push("**`prose`(" + (tally["prose"] || 0) + " 条,占 " + Math.round(((tally["prose"] || 0) / claims.length) * 100) + "%)可证伪性未定。** 这是本清册最重要的发现:近半数声称无法从文本本身看出「什么观测能推翻它」。一条不可证伪的声称既不能被证实也不能被证伪 —— 审计只有两条诚实出路:改写成可证伪的形式,或标记为不可判定。**不得默认它为真。**");
L.push("");
...
L.push("");
L.push("声称 AC 控制台暴露 “Interface and webhook management.”。实况:`apps/web/src/app/ac/[tenantId]/interface-management/page.tsx` 与 `.../webhook-management/page.tsx` 均只渲染 `AcBusinessRouteUnavailableScreen`。该声称**对 AC scope 为假**;tenant 侧同名路由仍为真 —— 原文缺 scope 限定,这是 `ui-surface` 类最常见的失效方式。");
L.push("");
L.push("**二、`docs/user-guide/public-presence/README.md:10`**");
L.push("");
L.push("声称 “Homepage/studio/editor pages.”。实况:`apps/web/src/app` 下 `studio/editor` 零命中,实际路径为 `/studio/public-presence/[tenantId]/[talentId]`。但原文究竟是否在指一条路径,本身即有歧义 —— 这正是 `prose` 类的典型病症:**连「它错了没有」都要先裁定它在说什么。**");
L.push("");
L.push("## 文件分布");
L.push("");
L.push("| 文件 | 声称数 |");
L.push("|---|---|");
for (const [f, arr] of Object.entries(byFile).sort((a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0])))
  L.push("| `" + f + "` | " + arr.length + " |");
L.push("");
L.push("## 全量清册");
L.push("");
...
if (process.argv.includes("--check")) {
  ...
  console.log(JSON.stringify({ ok: true, written: relative(REPO, OUT), claims: claims.length, files: files.length, tally }, null, 2));
}
