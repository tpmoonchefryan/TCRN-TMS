...
function mutate(name, file, apply, denomKey, note) {
  ...
  const b = before[denomKey], a = after[denomKey];
  const sawRed = b.sha !== a.sha;
  results.push({
    name, file, denominator: denomKey, note,
    before: `${b.n} @ ${b.sha.slice(0, 12)}`,
    after: `${a.n} @ ${a.sha.slice(0, 12)}`,
    sawRed, restoredClean,
    verdict: sawRed && restoredClean ? "RED-PROVEN" : sawRed ? "RED但还原失败" : "假绿 — 变异未被检出",
  });
  console.log(`${sawRed ? "RED " : "假绿"} ${name}: ${b.n}->${a.n}  还原${restoredClean ? "OK" : "失败"}`);
}
...
mutate(
  ...
  (s) => s.replace(/^  resource\(\s*\n?\s*'tenant\.manage'/m, "  resource(\n    'tenant.manage.MUTATED'"),
  "rbac",
  "catalog 资源码改名时 RBAC 分母须变"
);

writeFileSync(
  "docs/facts/_meta/red-proof-results.json",
  JSON.stringify(results, null, 2)
);
console.log("\n" + JSON.stringify(results.map(r => ({ n: r.name, v: r.verdict })), null, 2));
