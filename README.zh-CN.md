...
权限治理现在只有一个内置救援角色：`INITIAL_ADMIN`。日常角色均为自定义角色，通过 `/api/v1/roles` 使用能力包和可选的高级资源/动作覆盖进行编辑。旧的 `/api/v1/system-roles` 读取端点保留为兼容面；写入端点已废弃或阻止，角色删除因审计历史被禁用，角色激活/停用生命周期已移除。
