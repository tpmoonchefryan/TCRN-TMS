...
- [生产环境部署](#-生产环境部署)
- [自定义域名配置](#-自定义域名配置)
- [外部 PII Platform 边界](#-外部-pii-platform-边界)
- [API 参考](#-api-参考)
- [安全机制](#-安全机制)
- [许可证](#-许可证)
...
- **证明门槛下的写入路径**：源码支持 adapter-gated 的外部 PII 交接，但实时外部执行需要单独环境证明
- **门户边界**：PII 查看留在外部平台；没有脱敏 fixture 证明前，不把 SSO 或门户成功写成已验收的 TMS 流程
...
4. 源码支持在 effective `TCRN_PII_PLATFORM` adapter 存在时，以 `customerId` 做外部 PII 交接；实时外部执行仍需证明
5. PII 查看属于外部平台门户边界；TMS 不回读 PII，本 README 不验证外部 SSO 成功
...
# 5b. 如需了解 PII 流程，请先阅读外部 `TCRN_PII_PLATFORM` 证明边界
...
# 外部 PII Platform
# 本仓库仅记录 adapter-gated 边界
# 外部 adapter 凭证、SSO、门户和写透证明不在本 README 内验收
...
1. 源码支持仓库外的 adapter-gated 外部 PII 交接。
2. 实时 adapter 凭证、SSO、门户访问和外部权限校验不由本 README 验收。
3. 客户创建/编辑写透、门户跳转和 PII 报告交接都属于 proof-gated 的外部平台流程。
4. TMS 保持非 PII 客户核心系统角色，不把外部 PII 回读进自己的 UI。

### 操作证明边界

- 不要把本 README 当作 adapter 设置清单。
- 在具备一次性 fixture、脱敏、负向检查、清理和读回证明前，不发布 SSO、门户、写透或报告 handoff 操作流程。
- 不要把 integration 凭证、auth header、cookie、session id、payload 或客户敏感数据放入截图和支持工单。
...
本地开发只需要启动 TMS 主运行时。PII 外部流程演练在满足证明边界前不进入已验收指南。
...
TOTP 设置、停用、恢复码处理和租户强制策略仍是 `OKL-G19-ACCOUNT-TOTP-001` 下的 proof-gated 敏感流程。在具备脱敏 setup、恢复、清理和可访问性证明前，不把可见 UI 或源码支持当作已验收操作流程。
