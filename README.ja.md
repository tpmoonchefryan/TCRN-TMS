...
現在の UI 監査メモ：少なくとも 1 つのローカル AC テナント証拠では追加のデフォルトロール割り当てが表示されたため、User Guide は `INITIAL_ADMIN` のみのロールベースラインを既知の制限として記録し、ソース/UI 再確認で `GAP-P2-RBAC-001` が閉じられるまで過大に主張しません。
...
現在の UI 監査メモ：公開訪問者の投稿、CAPTCHA、フィード、追加読み込み、リアクション、復旧ワークフローは、`GAP-P2-PUBLIC-VISITOR-002` が閉じられるまで利用可能な手順として主張しません。
