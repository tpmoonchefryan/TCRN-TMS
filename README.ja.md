...
ロールガバナンスの組み込み救援ロールは `INITIAL_ADMIN` のみです。日常運用のロールはすべてカスタムロールとして `/api/v1/roles` から管理し、能力パックと必要に応じた高度なリソース/アクション上書きを使います。従来の `/api/v1/system-roles` 読み取りエンドポイントは互換用に残りますが、書き込みルートは廃止またはブロックされ、監査履歴のためロール削除は無効化され、ロールの有効/無効ライフサイクルも削除されています。
