...
describe('integration adapter config mutation schema', () => {
  ...
  it('validates batched update payloads with mixed mutation intents', () => {
    expect(
      UpdateAdapterConfigsSchema.parse({
        adapterVersion: 7,
        configs: [
          {
            configKey: 'api_key',
...
describe('integration definition-backed create schema', () => {
  ...
  it('accepts ai as a first-class adapter type and exposes one addable AI Adapter create definition', () => {
    ...
    expect(INTEGRATION_ADAPTER_CREATE_DEFINITIONS[0].configFields[0]).toMatchObject({
      ...
      input: 'select',
      secret: false,
      options: [{ value: 'OPENAI' }, { value: 'ANTHROPIC' }, { value: 'GEMINI' }],
    });
    expect(
      INTEGRATION_ADAPTER_CREATE_DEFINITIONS[0].aiProviders?.map((provider) => provider.provider)
    ).toEqual(['OPENAI', 'ANTHROPIC', 'GEMINI']);
    expect(
      INTEGRATION_ADAPTER_CREATE_DEFINITIONS.map((definition) => definition.key)
    ).not.toContain('bilibili-api-key');
    expect(
      INTEGRATION_ADAPTER_CREATE_DEFINITIONS.map((definition) => definition.key)
    ).not.toContain('youtube-oauth');
  });
