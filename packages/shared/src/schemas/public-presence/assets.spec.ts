...
describe('public presence D-026 asset runtime contracts', () => {
  ...
  it('validates artist lifecycle flow structure and homepage policy by stage', () => {
    expect(
      ArtistLifecycleFlowSchema.parse({
        nodes: [
          {
  ...
  it('parses template and component asset manifests using the runtime contract version', () => {
    expect(
      CurrentPublicPresenceAssetRuntimeVersionSchema.parse(PUBLIC_PRESENCE_ASSET_RUNTIME_VERSION)
    ).toBe(PUBLIC_PRESENCE_ASSET_RUNTIME_VERSION);

    expect(
      PublicPresenceTemplateAssetManifestSchema.parse({
        assetKind: 'template',
    ...
    ).toBe('activeTalentHub');

    expect(
      PublicPresenceComponentAssetManifestSchema.parse({
        assetKind: 'component',
        runtimeContractVersion: PUBLIC_PRESENCE_ASSET_RUNTIME_VERSION,
  ...
  it('parses immutable asset revisions and publish pins', () => {
    ...
    expect(
      PublicPresenceAssetRevisionPinSchema.parse({
  ...
  it('builds system component seeds and list entries that match the shared schemas', () => {
    ...
    expect(
      PublicPresenceAssetListEntrySchema.parse({
        asset: {
  ...
  it('produces a seeded asset catalog with both template and component authorities', () => {
    ...
    expect(
      seeds.some((seed) => seed.assetKind === 'template' && seed.templateId === 'activeTalentHub')
    ).toBe(true);
    expect(
      seeds.some((seed) => seed.assetKind === 'template' && seed.templateId === 'debutReveal')
    ).toBe(true);
    expect(
      seeds.some((seed) => seed.assetKind === 'component' && seed.componentType === 'SocialLinks')
    ).toBe(true);

    const templateSeed = seeds.find((seed) => seed.templateId === 'debutReveal');
    expect(
      PublicPresenceAssetDetailSchema.parse({
        asset: {
