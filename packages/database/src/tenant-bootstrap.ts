...
export async function copyTenantTemplateForeignKeys(
  ...
  const targetForeignKeys = targetConstraints.filter((constraint) => constraint.constraintType === 'f');
  ...
  for (const templateConstraint of templateForeignKeys) {
    ...
    targetBySignature.set(signature, {
      ...templateConstraint,
      definition: rewriteSchemaReference(templateConstraint.definition, 'tenant_template', schemaName),
    });
  }
}
