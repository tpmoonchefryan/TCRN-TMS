...
describe('UAT homepage fixtures', () => {
  ...
  it('covers every currently supported homepage component type with representative data', () => {
    ...
    ) as Record<(typeof HOMEPAGE_COMPONENT_TYPES)[number], (typeof fixture.content.components)[number]>;

    assert.equal(byType.ProfileCard.props.displayName, 'Sakura Ch.');
    assert.equal(
      Array.isArray(byType.SocialLinks.props.platforms) && byType.SocialLinks.props.platforms.length,
      3,
    );
    assert.equal(
    ...
    assert.match(String(byType.RichText.props.contentHtml), /canonical all-components homepage fixture/i);
